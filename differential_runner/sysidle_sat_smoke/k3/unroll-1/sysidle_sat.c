...
int main(int argc, char *argv[])
{
  ...
  /* Stress test. */
  i = 0;
  pthread_create(&tids[0], NULL, timekeeping_cpu, &ta_array[0]); i++;
  pthread_create(&tids[1], NULL, other_cpu, &ta_array[1]); i++;
  pthread_create(&tids[2], NULL, other_cpu, &ta_array[2]); i++;
