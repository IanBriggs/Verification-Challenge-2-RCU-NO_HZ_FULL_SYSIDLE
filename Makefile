...
SMACK_ARGS = --time-limit=36000 --pthread --unroll=6 --loop-limit=6 --context-bound=1 -v
...
.PHONY: sysidle_sat
sysidle_sat: 
	-mkdir -p gen/sysidle_sat && cd gen/sysidle_sat && $(SMACK) $(CURDIR)/src/sysidle_sat.c 2>&1

.PHONY: sysidle
sysidle: 
	-mkdir -p gen/sysidle && cd gen/sysidle && $(SMACK) $(CURDIR)/src/sysidle.c 2>&1
...
.PHONY: sysidle_smoke
sysidle_smoke: 
	-mkdir -p gen/sysidle_smoke && cd gen/sysidle_smoke && $(SMACK) $(CURDIR)/src/sysidle_smoke.c 2>&1

.PHONY: sysidle_sat_smoke
sysidle_sat_smoke: 
	-mkdir -p gen/sysidle_sat_smoke && cd gen/sysidle_sat_smoke && $(SMACK) $(CURDIR)/src/sysidle_sat_smoke.c 2>&1
